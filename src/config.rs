...
impl AppConfig {
    pub fn load() -> Self {
        ...
        if tenant_id.trim().is_empty() {
            panic!("[ARCH-COMPLIANCE] TENANT_ID is MANDATORY.");
        }
        ...
        {
            panic!("[ARCH-COMPLIANCE] Insecure HTTP target URLs are strictly forbidden. Use mTLS.");
        }

        let tls_ca_path =
            env::var("GRPC_TLS_CA_PATH").expect("[ARCH-COMPLIANCE] GRPC_TLS_CA_PATH missing.");
        let tls_cert_path = env::var("STREAM_GATEWAY_SERVICE_CERT_PATH")
            .expect("[ARCH-COMPLIANCE] STREAM_GATEWAY_SERVICE_CERT_PATH missing.");
        let tls_key_path = env::var("STREAM_GATEWAY_SERVICE_KEY_PATH")
            .expect("[ARCH-COMPLIANCE] STREAM_GATEWAY_SERVICE_KEY_PATH missing.");
